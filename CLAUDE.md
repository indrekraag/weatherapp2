...
### Three follow-up fixes (same day, after live phone testing)

1. **Sadu row → rain-probability %, not mm/h** (`7fcfc42`). A light-rain day (e.g. Friday 0.6 mm spread over 24 h ≈ 0.05 mm/h) made every precip bar collapse to the 10 % floor with near-invisible 0.2-alpha colour, so the popup looked empty even though the day tile said "0.6 mm". Switched the sheet's Sadu sparkline to hourly `precipitation_probability` (matches the main Sadu card's default; mm/h fallback when prob absent). mm total stays in the summary line; label is now `Sadu — tõenäosus %`. `_hourSliceForDay` gained a `prob` array.
2. **Invisible sparklines fixed** (`313030b`). The bars rendered into a bare `.dsheet-spark` (only position/height/width), so the `<span>` bars had no flex sizing and collapsed to zero width — **all three** charts were blank, not just precip. Fix: add the existing `.metric-spark` class alongside `.dsheet-spark` (gives `display:flex` + `.metric-spark>span` bar styling); `.dsheet-spark` keeps the taller 54 px height. **Lesson: `renderSpark` only populates bar heights — the container must be `.metric-spark` (or carry its flex + `>span` rules) or nothing shows.**
3. **Tap a bar → that hour's value** (`fb0fc0e`). Mobile has no hover tooltip, so per-hour figures were unreachable. New `_wireSparkTap(cid, data, hours, fmt, readoutId)`: tapping a bar highlights it (`.metric-spark>span.sel`) and writes `HH:MM · value` into a `.dsheet-val` readout in that row's label (e.g. `14:00 · 23°`, `14:00 · 42%`, `14:00 · 6 m/s`). Precip readout follows the row's mode. The slice now also carries an `hour[]` of `HH:MM` labels. Verified live: single-select per row, re-tap updates, 0 console errors.
